...
def load():
    part1, part2 = open("05.in").read().split("\n\n")
    lines = list(reversed(part1.split("\n")))
    num_stacks = int(lines[0].split()[-1])
    stacks = [[] for _ in range(num_stacks+1)]
    for l in lines[1:]:
        for i in range(num_stacks):
            if 4*i+1 < len(l) and l[4*i+1] != " ":
                stacks[i+1].append(l[4*i+1])
    moves = [
        tuple(int(v) for v in re.findall("\d+", l))
        for l in part2.split("\n")
        if len(l) > 0
    ]
    return stacks, moves

def move(stacks, num, from_, to):
    for _ in range(num):
        stacks[to].append(stacks[from_].pop())

def solve():
    stacks, moves = load()
    for num, from_, to in moves:
        move(stacks, num, from_, to)
