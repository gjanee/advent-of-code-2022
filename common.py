...
def do_until_true(fn):
    """Repeatedly call a function until it returns True.  Return the
    number of False calls or, equivalently, the index of the call that
    returned True if the sequence of calls is thought of as an
    iterable.
    """
    i = 0
    while True:
...
def minmax(iterable):
    """Return the minimum and maximum values of an iterable in one call."""
    l = list(iterable)
    return min(l), max(l)

def cmp(a, b):
    """Return -1, 0, 1 if a > b, a == b, or a < b.  From Python 2."""
    return (a < b) - (a > b)

def pick(fn, iterable):
    """Return the (first) item of an iterable that matches a predicate."""
    return next(filter(fn, iterable))

def lmap(*args):
    """Materialize a map iterator as a list."""
    return list(map(*args))

def lfilter(*args):
    """Materialize a filter iterator as a list."""
