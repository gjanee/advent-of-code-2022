...
class Directory(FilesystemEntry):
    ...
    def child(self, name):
        return pick(lambda c: c.name == name, self.children)

    def add_child(self, line):
        if line.startswith("dir"):
            e = Directory(line.split()[1])
        else:
            size, name = line.split()
            e = File(name, int(size))
        self.children.append(e)

    def total_size(self, fn):
        # Return the total size of the directory.  Also call `fn`
        # with the total size.
        s = sum(c.total_size(fn) for c in self.children)
        fn(s)
        return s
    ...
    if line.startswith("$"):
        if line.startswith("$ cd"):
            ...
            if name == "/":
                ...
                path.append(path[-1].child(name))
        else:
            pass  # nothing to do on ls command
    else:
        path[-1].add_child(line)
...
total_size = root.total_size(accum)
print(part1_sum)
...
# thus the total amount of used space) is 48381165; this means that
# the size of the unused space must currently be 21618835, which isn't
...
# --------------------
#
# So the space used must be less than or equal to 40000000.

part2_size = total_size

def select_smallest(size):
    global part2_size
    if total_size-size <= 40000000:
        part2_size = min(part2_size, size)

root.total_size(select_smallest)
print(part2_size)
