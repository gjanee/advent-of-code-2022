...
# fun, the path any new sand takes before falling forever is shown
# here with ~:
#
# .......+...
# .......~...
...
from common import cmp, do_until_true

class Coord(namedtuple("Coord_base", "x y")):

    def __add__(self, other):
        return Coord(self.x+other.x, self.y+other.y)

    def delta(self, other):
        # Return a delta such that repeatedly adding it to `self`
        # eventually yields `other`.
        return Coord(cmp(self.x, other.x), cmp(self.y, other.y))
...
ROCK, AIR, SAND = range(3)

# collections.defaultdict will not allow us to override __getitem__ in
# part 2 below, so we define our own class.

class Grid(dict):
    def __getitem__(self, coord):
        return self.get(coord, AIR)

grid = Grid()

for path in open("14.in"):
    coords = [
        Coord(*[int(v) for v in c.split(",")])
        for c in path.split("->")
    ]
    for i in range(len(coords)-1):
        c = coords[i]
        delta = coords[i].delta(coords[i+1])
        grid[c] = ROCK
        while c != coords[i+1]:
            c += delta
            grid[c] = ROCK

max_depth = max(c.y for c in grid)

def drop_grain(start, termination_fn=None):
    # Drop a grain of sand from `start` and return the coordinate of
    # its resting position.  Call `termination_fn`, if supplied, on
    # each coordinate; if the function returns True, return None.
    c = start
    while True:
        if termination_fn != None and termination_fn(c):
            return None
        if grid[c.below] == AIR:
            c = c.below
        elif grid[c.below_left] == AIR:
            c = c.below_left
        elif grid[c.below_right] == AIR:
            c = c.below_right
        else:
            grid[c] = SAND
            return c

num_grains = do_until_true(
    lambda: drop_grain(Coord(500, 0), lambda c: c.y > max_depth) == None
)
print(num_grains)
...
Grid.__getitem__ = (
    lambda self, c: ROCK if c.y == max_depth+2 else self.get(c, AIR)
)

print(
    num_grains
    + do_until_true(lambda: drop_grain(Coord(500, 0)) == Coord(500, 0)) + 1
)
