...
# each step, you'll need to update the position of the tail if the
...
# --------------------
#
# Run this program with a "visualize" argument to see the first 1000
# steps of the rope's movement in part 2 animated.

from collections import namedtuple

class Coordinate(namedtuple("Coordinate_base", "r c")):

    def is_adjacent(self, other):
        return max(abs(other.r-self.r), abs(other.c-self.c)) <= 1

    def move(self, dir):
        # Move the head.
        if dir in ["L", "R"]:
            r = self.r
            c = self.c + (1 if dir == "R" else -1)
        else:
            r = self.r + (1 if dir == "D" else -1)
            c = self.c
        return Coordinate(r, c)

    def catch_up(self, head):
        # Catch the tail up to the head.  The head can be at most 2
        # units away along either axis, hence the average of the
        # coordinates yields the correct movement.  In part 1 the head
        # moves only up/down/left/right, but in part 2 it can move
        # diagonally.  The code below works in both cases.
        if abs(head.r-self.r) > 1:
            r = (head.r + self.r)//2
        else:
            r = head.r
        if abs(head.c-self.c) > 1:
            c = (head.c + self.c)//2
        else:
            c = head.c
        return Coordinate(r, c)

head = tail = Coordinate(0, 0)
visited = set([tail])
...
L = 10  # rope length
rope = [Coordinate(0, 0) for _ in range(L)]  # head is at 0, tail at L-1
visited = set([rope[L-1]])
history = [rope.copy()]
...
for line in open("09.in"):
    ...
    for _ in range(int(n)):
        ...
        for i in range(1, L):
            if not rope[i].is_adjacent(rope[i-1]):
                rope[i] = rope[i].catch_up(rope[i-1])
                if i == L-1:
                    visited.add(rope[i])
        history.append(rope.copy())

print(len(visited))

import sys
if not (len(sys.argv) >= 2 and sys.argv[1] == "visualize"):
    sys.exit()
...
H = 1000  # history limit
rmin = rmax = cmin = cmax = 0
for rope in history[:H]:
    for knot in rope:
        rmin, rmax = min(rmin, knot.r), max(rmax, knot.r)
        cmin, cmax = min(cmin, knot.c), max(cmax, knot.c)
R, C = rmax-rmin+1, cmax-cmin+1  # grid dimensions
...
for rope in history[:H]:
    ...
    for _ in range(R):
        stdscr.addstr("."*C + "\n")
    for knot in visited:
        stdscr.addch(knot.r-rmin, knot.c-cmin, "*")
    for i in range(L-1, -1, -1):
        stdscr.addch(
            rope[i].r-rmin,
            rope[i].c-cmin,
            "H" if i == 0 else "T" if i == L-1 else "O"
        )
    stdscr.move(rope[0].r-rmin, rope[0].c-cmin)
    visited.add(rope[L-1])
    stdscr.refresh()
    sleep(.1)
