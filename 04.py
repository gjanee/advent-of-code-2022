...
# Elves pair up and make a big list of the section assignments for
...
#   got sections 5, 6, and 7, while the other also got 7, plus 8 and
#   9.
#
# This example list uses single-digit section IDs to make it easier to
# draw; your actual list might contain larger numbers.  Visually,
...
pairs = [
    tuple(set(to_range(spec)) for spec in line.strip().split(","))
    for line in open("04.in")
]

print(sum(p[0] <= p[1] or p[1] <= p[0] for p in pairs))
...
print(sum(len(p[0] & p[1]) > 0 for p in pairs))
