...
# out of your pack, but the amount of water slowly draining from a big
# crack in its screen tells you it probably won't be of much immediate
# use.
#
...
# register) during the 20th cycle and every 40 cycles after that (that
...
#   of the second addx -1, so the value of register X is the starting
#   value, 1, plus all of the other addx values up to that point:
...
def run(fn):
    # Run the program to completion, calling fn(cycle, x) on each
    # cycle.
...
# that, and so on.  The left-most pixel in each row is in position 0,
...
# --------------------
#
# Helpfully, the number of cycles required by the program exactly
# matches one complete scan of the CRT beam.
