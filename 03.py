...
# that did the packing failed to follow this rule for exactly one item
# type per rucksack.
#
...
def priority(type):
    return ord(type) - (ord("a")-1 if type.islower() else ord("A")-27)

sacks = open("03.in").read().split()

print(
    sum(
        priority((set(s[:len(s)//2]) & set(s[len(s)//2:])).pop())
        for s in sacks
    )
)
...
# one is by finding the one item type that is common between all three
# Elves in each group.
#
# Every set of three lines in your list corresponds to a single group,
# but each group can have a different badge item type.  So, in the
# above example, the first group's rucksacks are the first three
# lines:
#
# vJrwpWtwJgWrhcsFMMfFFhFp
# jqHRNqRjqzjGDLGLrsFMfFZSrLrFZsSL
...
print(
    sum(
        priority((set(sacks[i]) & set(sacks[i+1]) & set(sacks[i+2])).pop())
