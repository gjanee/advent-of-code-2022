...
# comes over to interpret.  As it turns out, they overheard you
# talking about trying to find the grove; they can show you a shortcut
# if you answer their riddle.
#
...
#   bbbb to yell each of their numbers; the monkey then yells the sum
...
# already has its other number, 5, from dbpl.  This allows it to yell
# its own number by finding 30 multiplied by 5: 150.
#
...
class Monkey:
    ...
    def __init__(self, *args):
        ...
        if len(args) == 2:
            ...
            self.left_name, self.op, self.right_name = args[1:4]
        Monkey.all[self.name] = self

    @property
    def is_tree(self):
        return hasattr(self, "left_name")

    @property
    def left(self):
        return Monkey[self.left_name]

    @property
    def right(self):
        return Monkey[self.right_name]

    def eval(self):
        if self.is_tree:
            return OP[self.op](self.left.eval(), self.right.eval())
        else:
            return self.value
    ...
    def has_humn(self):
        return (
            self.name == "humn"
            or (self.is_tree and (self.left.has_humn or self.right.has_humn))
        )

for line in open("21.in"):
    name, expr = line.strip().split(": ")
    m = re.match("^([a-z]+) ([-+*/]) ([a-z]+)$", expr)
    if m:
        Monkey(name, m[1], m[2], m[3])
    else:
        Monkey(name, int(expr))

print(Monkey["root"].eval())
...
# isn't a monkey - it's you.  Actually, you got the job wrong, too:
...
# --------------------
#
# We solve this exactly as we would by hand, by isolating humn through
# algebraic manipulations.  For example, given htree + ltree = rtree,
# where htree is a tree containing humn and ltree and rtree are other
# evaluation trees, we create a new Monkey node to express
# htree = rtree - ltree.  Eventually we reach a state humn = rtree and
# from there evaluate rtree.
#
# We verified that all (integer) divisions are perfect, and hence
# are invertible.

from itertools import count

left, right = Monkey["root"].left, Monkey["root"].right
if Monkey["root"].right.has_humn:
    left, right = right, left

name_generator = count(1)

while left.name != "humn":
    if left.op == "+":
        htree, ltree = left.left, left.right
        if left.right.has_humn:
            htree, ltree = ltree, htree
        right = Monkey(next(name_generator), right.name, "-", ltree.name)
        left = htree
    elif left.op == "-" and left.left.has_humn:
        right = Monkey(next(name_generator), right.name, "+", left.right.name)
        left = left.left
    elif left.op == "-" and left.right.has_humn:
        right = Monkey(next(name_generator), left.left.name, "-", right.name)
        left = left.right
    elif left.op == "*":
        htree, ltree = left.left, left.right
        if left.right.has_humn:
