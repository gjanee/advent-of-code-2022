...
# a large network of subterranean tunnels.  You don't have time to
...
# beacons that just happen to not be the closest beacon to any sensor.
...
sensors = []
beacons = set()  # beacons may be referred to more than once in the input
for line in open("15.in"):
    sx, sy, bx, by = [int(v) for v in re.findall("-?\d+", line)]
    sensors.append(Sensor(sx, sy, distance(sx, sy, bx, by)))
    beacons.add(Beacon(bx, by))

Y = 2000000  # row of interest

coverage = set()
for s in sensors:
    d = s.radius - abs(s.y-Y)
    coverage.update(range(s.x-d, s.x+d+1))

# Subtle point: any beacons within `coverage` must be excluded.
# Because every beacon in the row of interest is in some sensor's
# range, necessarily every such beacon is included in `coverage`, so
# we can just subtract the number of beacons in that row.

print(len(coverage) - sum(b.y == Y for b in beacons))
...
# --------------------
#
# The puzzle hints that the answer can be found by applying the test
# from part 1, i.e., by looking for a gap in coverage row by row.  Our
# initial approach did this and it works, but it's slow.  Instead, we
# rotate the grid 45 degrees so that sensor coverage regions change
# from diamonds to axis-aligned squares.  The rotation matrix is
#
#    [   1    1]
#    [  -1    1]
#
# (we ignore the factor of 1/sqrt(2) here).  The inverse rotation
# matrix is
#
#    [ 1/2 -1/2]
#    [ 1/2  1/2]
#
# (the factor of 1/2 makes up for our previous ignorance of
# 1/sqrt(2)).  Note that radii do not change under rotation, even when
# ignoring the 1/sqrt(2) factor (it helps to confirm this on graph
# paper).
#
# Since sensor coverage regions are now axis-aligned, we do not need
# to look at every row, but only transitional rows where some sensor's
# region either starts or ends.  Specifically, if a sensor's vertical
# range is [start, end), then we look at rows y=start and y=end.
#
# We assume that the uncovered position is not in one of the very
# corners of the 4000001x40000001 region.

class RotatedSensor(namedtuple("RotatedSensor_base", "xrange yrange")):
    def __new__(cls, s):
        x, y = s.x+s.y, -s.x+s.y  # rotate
        return super().__new__(
            cls,
            range(x-s.radius, x+s.radius+1),
            range(y-s.radius, y+s.radius+1)
        )

sensors = [RotatedSensor(s) for s in sensors]

def find_gap(ranges):
    # Return the single interstitial value not covered by `ranges`, or
    # None.
    l = sorted(ranges, key=lambda r: (r.start, r.stop))
    hwm = l[0].start  # high water mark
    for r in l:
        if r.start > hwm:
            assert r.start == hwm+1
            return hwm
        hwm = max(hwm, r.stop)
    return None

transitional_rows = set(
    [s.yrange.start for s in sensors]
    + [s.yrange.stop for s in sensors]
)

# Below, we exclude the bottom-most row since there is necessarily no
# coverage there.

for y in sorted(transitional_rows)[:-1]:
    x = find_gap(s.xrange for s in sensors if y in s.yrange)
    if x != None:
        break

x, y = (x-y)//2, (x+y)//2  # unrotate
print(x*4000000+y)
