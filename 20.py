...
# --------------------
#
# Another tricky puzzle, first because numbers are repeated in the
# input (they must still be moved individually), and second because if
# there are N numbers, there are only N-1 gaps into which those
# numbers can be moved, and thus index computations are modulo N-1,
# not N.

from collections import namedtuple
from common import pick

Number = namedtuple("Number", "seq val")

nums_visit_order = [
    Number(seq, int(val))
    for seq, val in enumerate(open("20.in"))
]

def solve(repeats):
    nums = nums_visit_order.copy()
    for _ in range(repeats):
        for n in nums_visit_order:
            if n.val == 0:
                continue
            i = nums.index(n)
            new_i = (i+n.val)%(len(nums)-1)
            # The following two lines work whether `new_i` precedes or
            # follows `i`.  Best to draw it out to be convinced of
            # that.
            del nums[i]
            nums.insert(new_i, n)
    zero_i = pick(lambda i: nums[i].val == 0, range(len(nums)))
    def offset_val(offset):
        return nums[(zero_i+offset)%len(nums)].val
    return offset_val(1000) + offset_val(2000) + offset_val(3000)
...
# 0, 811589153, 2434767459, 3246356612, 1623178306, -1623178306,
# -2434767459
#
# After 4 rounds of mixing:
# 0, 1623178306, -2434767459, 811589153, 2434767459, 3246356612,
# -1623178306
#
# After 5 rounds of mixing:
# 0, 811589153, -1623178306, 1623178306, -2434767459, 3246356612,
# 2434767459
#
...
# 0, -2434767459, 2434767459, 1623178306, -1623178306, 811589153,
# 3246356612
#
# After 8 rounds of mixing:
# 0, 1623178306, 3246356612, 811589153, -2434767459, 2434767459,
# -1623178306
#
# After 9 rounds of mixing:
# 0, 811589153, 1623178306, -2434767459, 3246356612, 2434767459,
# -1623178306
#
...
nums_visit_order = [Number(n.seq, n.val*811589153) for n in nums_visit_order]
