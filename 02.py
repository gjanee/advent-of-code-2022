...
# contains many rounds; in each round, the players each simultaneously
# choose one of Rock, Paper, or Scissors using a hand shape.  Then, a
# winner for that round is selected: Rock defeats Scissors, Scissors
# defeats Paper, and Paper defeats Rock.  If both players choose the
...
# --------------------
#
# We represent shapes as integers modulo 3.  Shape A defeats shape B
# if A is B's circular successor.
...
def score(opponent, you):
    if you == succ(opponent):    # win
        v = 6
    elif opponent == succ(you):  # lose
        v = 0
    else:                        # draw
        v = 3
    return you+1 + v
...
total = 0
for round in open("02.in"):
    opponent, you = [mapping[v] for v in round.split()]
